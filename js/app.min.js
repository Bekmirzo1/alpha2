...
// *Aside instruction setting
const instrAside = document.querySelector('.instr__aside');
if (instrAside) {
    // *Delete _side-wrapper
    function delSideWrapper(media) {
        if (media.matches) {
            instrAside.classList.remove('_side-wrapper')
            // mobAside();
        } else {
            instrAside.classList.add('_side-wrapper')
        }
    }
    const tabletMedia = window.matchMedia('(max-width: 1023.98px)');
    tabletMedia.addListener(delSideWrapper);
    delSideWrapper(tabletMedia);

    // *Mobile aside
    function mobAside(media) {
        if (media.matches) {
            const valueAside = instrAside.querySelector('.aside-instr__value');
            if (!_is_hidden(valueAside)) {
                const valueAsideText = valueAside.querySelector('span');
                const asideBody = document.querySelector('.aside-instr__body');
                instrAside.addEventListener('click', function (e) {
                    const target = e.target;
                    if (target.closest('.aside-instr__value')) {
                        asideBody.classList.add('_active');
                        document.body.classList.add('_lock');
                    } else if (target.closest('.aside-instr__close') || target.classList.contains('aside-instr__bg')) {
                ...
                function valueContent() {
                    // const itemActive = instrAside.querySelector('.aside-instr__item._active');
                    const itemActive = instrAside.querySelector('.aside-instr__item a._active');
                    valueAsideText.textContent = itemActive.textContent;
                }
                valueContent();
            }
        }
    }
    const mobMedia = window.matchMedia('(max-width: 767.98px)');
    mobMedia.addListener(mobAside);
    mobAside(mobMedia);
}

// *Add spoller class on menu links
const menuLinks = document.querySelectorAll('.menu__link');
if (menuLinks.length > 0) {
    menuLinks.forEach(menuLink => {
        if (menuLink.hasAttribute('data-spoller')) {
            menuLink.classList.add('_spoller');
        }
    });
}
...
if (searches.length > 0) {
    for (let index = 0; index < searches.length; index++) {
        ...
        const closeIcon = search.querySelector('.search__close');
        closeIcon.addEventListener('click', function () {
            input_focus_remove(input);
            input.value = "";
            iconShow();
        });
        input.addEventListener('input', iconShow);
        function iconShow() {
            if (input.value == "") {
                closeIcon.classList.remove('_show');
            } else {
                closeIcon.classList.add('_show');
            }
        }
        iconShow();
    }
...
// *textareaDynamic
const textareaDynamics = document.querySelectorAll('._textarea-dynamic');

if (textareaDynamics.length > 0) {
    for (let index = 0; index < textareaDynamics.length; index++) {
        const textareaDynamic = textareaDynamics[index];
        const textarea = textareaDynamic.querySelector('textarea');
        const textareaCopy = textareaDynamic.querySelector('p');
        textarea.addEventListener('input', textareaAutoHeight);
        function textareaAutoHeight() {
            textareaCopy.textContent = textarea.value;
            const textareaStyles = window.getComputedStyle(textarea);
            const textareaLineHeight = textareaStyles.getPropertyValue('line-height');
            const textareaWidth = textarea.clientWidth;

            textareaCopy.style.lineHeight = textareaLineHeight;
            textareaCopy.style.fontFamily = textareaStyles.getPropertyValue('font-family');
            textareaCopy.style.fontSize = textareaStyles.getPropertyValue('font-size');
            textareaCopy.style.minHeight = textareaLineHeight;
            // textareaCopy.style.minHeight = textareaLineHeight;
            textareaCopy.style.width = textareaWidth + 'px';

            const defference = Math.round(textareaCopy.offsetHeight / parseFloat(textareaLineHeight));
            if (defference < 101) {
                textarea.style.height = textareaCopy.offsetHeight + 'px';
...
window.addEventListener("load", function () {
    ...
    if (headerSearch) {
        ...
        if (search) {
            let blockClass;
            headerSearch.setAttribute('href', '.search')
            const href = headerSearch.getAttribute('href');
            headerSearch.addEventListener('click', function (e) {
                const target_block = document.querySelector(href);
...
if (document.querySelector('.aside-instr__body-content')) {
    const bodyContentAside = document.querySelector('.aside-instr__body-content');
    let slider;
    function swiperApadt(media) {
        if (media.matches) {
            bodyContentAside.classList.add('_slider')
            slider = new Swiper('.aside-instr__body-content', {
                /*
                effect: 'fade',
                fadeEffect: {
                    crossFade: true
            ...
            if (bodyContentAside.classList.contains('swiper-container-initialized')) {
                slider.destroy();
            }
            bodyContentAside.classList.remove('_slider');
        }
    }
    const sliderMedia = window.matchMedia('(max-width: 1023.98px) and (min-width: 767.98px)');
    sliderMedia.addListener(swiperApadt);
    swiperApadt(sliderMedia);
}
...
function _goto(target_block, speed, offset = 0) {
	const moveTo = new MoveTo({
		tolerance: offset + document.querySelector('header').offsetHeight,
		duration: speed,
		easing: 'easeOutQuart'
	});
...
const item = document.querySelector('.dictionary__search');
if (item) {
	const parent_original = document.querySelector('.dictionary__header');
	const parent = document.querySelector('.dictionary__search-mob');
